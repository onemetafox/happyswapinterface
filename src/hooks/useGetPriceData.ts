...
import priceContracts from '../constants/elxrPriceContracts'
    ...
    const fetchData = async () => {
      try {
        if(multicallContract){
          const {elxrAddress, busdAddress, lpAddress} = priceContracts;
          const calls = [
            [elxrAddress, ERC20_INTERFACE.encodeFunctionData("balanceOf", [lpAddress])],
